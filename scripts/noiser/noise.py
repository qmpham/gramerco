...
def misspell(word):
    ...
    if r < 1.0/4: ### add extra character in position k [0, len(word)]
        ...
        l = word.pop(k)
    elif r < 3.0/4: ### swap characters in positions k and k+1, k in [0, len(word)-2]
...
def noise_line(toks,txt2inf,vocab,wrd2wrds_same_lemma,wrd2wrds_homophones,lempos2wrds,seen,args):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--w_inflect', type=float, default=5, help='Weight of inflecting a word (5)')
    ...
    wrd2wrds_same_lemma, wrd2wrds_homophones, lempos2wrds = read_lexicon(args.lexicon,vocab)
    tic = time.time()
    nsents = 0
    ntokens = 0
    ...
    for f in args.files:
        ...
        with open(f) as fd:
            ...
            for l in fd:
                ...
                noise_line(toks,txt2inf,vocab,wrd2wrds_same_lemma,wrd2wrds_homophones,lempos2wrds,seen,args)
                nsents += 1
                ntokens += len(toks)
    toc = time.time()
