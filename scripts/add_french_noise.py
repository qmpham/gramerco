...
def do_lexicon(toks, tags, lex_rep):
    ...
    for lex in lex_rep: #lex is: $VER￨-￨-￨ind￨pre￨3s:chutes￨chute
        tag, others = lex.split(':')
        others = others.split(separ)
        curr = others.pop()
        logging.debug('tag={} curr={} others={} n_found={} toks={}'.format(tag,curr,others,toks.count(curr),toks))
...
def noise_line(toks,lex,dic,rep,app,seen,args):
    n_attempts = 0
    n_changes = 0
    tags = [keep for t in toks]
    output(toks, tags) ### prints without noise
...
if __name__ == '__main__':
    ...
    parser.add_argument('-dic', '--dictionary_file', help='File with list of words and their frequencies')
    ...
    parser.add_argument('--p_cas', type=float, default=0.01, help='Prob of changing the first char case (def 0.01)')
